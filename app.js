...
function renderEnvelopes(){
 const c=current(); let h=pageHead("🏠 الأظرف / بنود المصروفات","في كل ظرف يمكنك تسجيل المصروفات بالتاريخ، أو إدخال الفعلي يدويًا في نهاية الدورة إذا لم تسجل مصروفات.",c&&!c.closed?`<button class="btn btn-primary btn-sm" onclick="openEnvelope()">＋ إضافة بند</button>`:"");
 ...
 h+=`<div class="cards" style="grid-template-columns:repeat(4,1fr)">${metric("📋","المخطط",money(cyclePlanned(c)))}${metric("💵","مخصص إضافي للبيت",money(cycleEnvelopeAllocations(c)))}${metric("💸","الفعلي",money(cycleActual(c)))}${metric("💰","الفائض / العجز",money(surplus),`العجز ${money(deficit)}`)}</div>`;
 ...
 ${c.envelopes.map(e=>{const a=envelopeActual(e), d=envelopeDelta(e), count=(e.expenses||[]).length;return `<tr><td><strong>${esc(e.name)}</strong>${count?`<div class="muted" style="font-size:11px">🧾 ${count} مصروف مسجل</div>`:""}</td><td>${money(e.planned)}</td><td>${a===null?"—":money(a)}</td><td class="${a===null?"muted":d>=0?"money-positive":"money-negative"}">${a===null?"لم يُدخل بعد":`${d>=0?"+":""}${money(d)}`}</td><td><div class="action-row">${!c.closed?`<button class="btn btn-primary btn-sm" onclick="openExpense('${e.id}')">＋ مصروف</button>`:""}<button class="btn btn-secondary btn-sm" onclick="openEnvelope('${e.id}')">تعديل</button>${!c.closed?`<button class="btn btn-danger btn-sm" onclick="deleteEnvelope('${e.id}')">حذف</button>`:""}</div></td></tr>`}).join("")||`<tr><td colspan="5" class="empty">لا توجد بنود.</td></tr>`}
 </tbody></table></div>
 ...
 if(c.envelopes.some(e=>(e.expenses||[]).length)){
   h+=`<div class="card" style="margin-top:18px"><div class="section-title"><h3>🧾 مصروفات الأظرف المسجلة</h3><span class="muted">الفعلي هنا محسوب تلقائيًا من المصروفات</span></div>`;
   h+=c.envelopes.filter(e=>(e.expenses||[]).length).map(e=>`<div class="expense-block"><div class="section-title"><strong>${esc(e.name)}</strong><strong>${money(envelopeActual(e))}</strong></div><div class="list">${e.expenses.map(x=>`<div class="list-row"><span>${esc(x.date)} — ${esc(x.description)}</span><span><strong>${money(x.amount)}</strong>${!c.closed?` <button class="btn btn-danger btn-sm" onclick="deleteExpense('${e.id}','${x.id}')">حذف</button>`:""}</span></div>`).join("")}</div></div>`).join("");
   h+=`</div>`;
 }
