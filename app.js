...
function cycleName(c){return c?.name || ("دورة "+(c?.startDate||""))}
...
function cycleAllocated(c){return cyclePlanned(c)+savingsByMethod(c,"fawry")+savingsByMethod(c,"gold_investment")}
...
function renderEnvelopeMini(c){
 ...
 return `<div class="table-wrap"><table class="table"><thead><tr><th>البند</th><th>المخطط</th><th>الفعلي</th><th>فائض/عجز</th></tr></thead><tbody>${c.envelopes.map(e=>`<tr><td>${esc(e.name)}</td><td>${money(e.planned)}</td><td>${money(e.actual ?? 0)}</td><td class="${envelopeDelta(e)>=0?"money-positive":"money-negative"}">${envelopeDelta(e)>=0?"+":""}${money(envelopeDelta(e))}</td></tr>`).join("")}</tbody></table></div>`;
}
...
function renderIncome(){
 const c=current(); let h=pageHead("💵 مصادر الدخل","مكان واحد فقط لإضافة وتعديل وحذف الدخل",c&&!c.closed?`<button class="btn btn-primary btn-sm" onclick="openIncome()">＋ إضافة مصدر دخل</button>`:"");
...
function renderEnvelopes(){
 ...
 h+=`<div class="cards" style="grid-template-columns:repeat(3,1fr)">${metric("📋","المخطط",money(cyclePlanned(c)))}${metric("💸","الفعلي",money(cycleActual(c)))}${metric("💰","الفائض / العجز",money(surplus),`العجز ${money(deficit)}`)}</div>`;
...
function openNewCycle(){
 ...
 <div class="form-group"><label>اسم الدورة</label><input name="name" placeholder="مثال: دورة أغسطس" required></div>
 ...
 ${prev?`<div class="card" style="margin-top:14px;background:var(--surface2)"><strong>📋 بنود الدورة السابقة ستظهر كمقترحات بعد الإنشاء.</strong><p class="muted">لن يتم اعتمادها إلا بعد مراجعتها والضغط على "اعتماد الدورة الجديدة".</p></div>`:""}`,d=>{
   const c={id:uid(),name:d.get("name"),startDate:d.get("startDate"),endDate:null,closed:false,approved:false,income:[],envelopes:prev?prev.envelopes.map(e=>({id:uid(),name:e.name,planned:e.planned,actual:null,actualEntered:false,proposed:true})):[],savings:[],fawryOpening:num(d.get("fawryOpening"))};
